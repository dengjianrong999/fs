...
<template>
	...
	<div class="fs-content">
		...
		<div class="input-box">
			...
			<div class="input-group">
				<span class="input-group-addon">工作场所名称：</span>
				<input type="text" class="form-control" v-model='zsgcValue2'>
			</div>
			<div class="input-group">
				<span class="input-group-addon">工作场所地址：</span>
				<input type="text" class="form-control" v-model='zsgcValue3'>
			</div>
			<div class="input-group">
				<span class="input-group-addon">工作场所等级：</span>
			...
			<!--按钮-->
			<div class="input-group">
				<span class="btn_content btn_query" @click="search()">
				...
				<span class="btn_content btn_empty" @click='clearAll()'>
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col">工作场所名称</th>
						<th scope="col">工作场所地址</th>
						<th scope="col" width="100px">工作场所等级</th>
						<th scope="col" width="80px">负责人</th>
						<th scope="col">操作</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item,index) of items">
						...
						<td :title="item.workplaceSite">
							{{item.workplaceSite}}
						</td>
						<td width="100px" :title="item.workplaceGrade">
							{{item.workplaceGrade}}
						</td>
						<td width="80px" :title="item.responsible">
							{{item.responsible}}
						</td>
						<td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				let temData = {
					...
					workplaceSite: this.zsgcValue3,
					workplaceGrade: this.zsgcValue4,
					page: this.page,
					rows: this.rows
				};
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}WorkplaceInfo/listPageJson`,
							params: temData
						})
						.then(function(res) {
			...
			derive() {
				...
				_this.$http({
					method: 'get',
					url: '/bjsy-jdc/fs/WorkplaceInfo/export',
					params: temData,
					responseType: 'blob'
				}).then(res => {
					_this.deriveAll(res);
				...
				layer.confirm(
					...
					function(index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "WorkplaceInfo/delete/" + id
							})
							.then(function(res) {
								_this.getpage();
							})
