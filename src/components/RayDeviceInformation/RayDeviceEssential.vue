	...
	<div class="fs-content">
		...
		<div class="input-box">
			...
			<div class="input-group">
				...
				<input type="text" class="form-control" v-model='zsgcValue2' @keyup.enter="select()">
			</div>
			<div class="input-group">
				<span class="input-group-addon">射线装置类别：</span>
				<input type="text" class="form-control" v-model='zsgcValue3' @keyup.enter="select()">
			</div>
			<div class="input-group">
				<span class="input-group-addon">活动种类：</span>
				<input type="text" class="form-control" v-model='zsgcValue4' @keyup.enter="select()">
			</div>
			<div class="input-group">
			</div>
			<!--按钮-->
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col" width="110px">射线装置数量</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key='index'>
						...
						<td width="110px" :title="item.deviceCategory">
							{{item.deviceCategory}}
						</td>
						<td width="110px" :title="item.deviceNumber">
							{{item.deviceNumber}}
						</td>
		...
		methods: {
			...
			update() {
				...
				layer.open({
					...
					content: ['#/RayDeviceEssentialWindow/' + id],
					end: function() {
						_this.getpage();
					}
				});
			...
			derive() {
				...
				_this.$http({
					method: 'get',
					url: '/bjsy-jdc/fs/radialdevice/export',
					params: temData,
					responseType: 'blob'
				}).then(res => {
					_this.deriveAll(res);
				...
				layer.confirm(
					...
					function(index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "radialdevice/delete/" + id
							})
							.then(function(res) {
								_this.getpage();
							})
