			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col" width="130px">年最大用量</th>
				...
				<tbody>
					<tr v-for="(item,index) of items" :key="index">
						...
						<td width="130px" :title="item.annualMaximum">
							{{item.annualMaximum}}
						</td>
	...
	export default {
		...
		methods: {
			...
			lastInterface() {
				let _this = this;
				_this.$http
					.get(`${_this.baseurl}unitInfo/listJson`)
					.then(function(res) {
			...
			update() {
				...
				let layerIndex = layer.open({
					...
					content: ['#/MaterialEssentialWindow/' + id],
					end: function() {
						_this.getpage();
					}
				});
			...
			derive() {
				...
				_this.$http({
					method: 'get',
					url: '/bjsy-jdc/fs/NontightInfo/export',
					params: temData,
					responseType: 'blob'
				}).then(res => {
					_this.deriveAll(res);
				...
				layer.confirm(
					...
					function(index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "NontightInfo/delete/" + id
							})
							.then(function(res) {
								_this.getpage();
							})
