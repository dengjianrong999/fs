			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th scope="col" width="70px">总活度</th>
						<th scope="col" width="70px">频次</th>
						<th scope="col">用途</th>
						<th scope="col" width="200px">来源/去向</th>
				...
				<tbody>
					<tr v-for="(item,index) of items">
						...
						<td width="70px" :title="item.totalActivity">
							{{item.totalActivity}}
						</td>
						<td width="70px" :title="item.frequency">
							{{item.frequency}}
						</td>
						<td :title="item.purpose">
							{{item.purpose}}
						</td>
						<td width="200px" :title="item.sourceTo">
							{{item.sourceTo}}
						</td>
		...
		methods: {
			...
			getpage() { // 按页获取数据
				...
				let temData = {
					...
					purpose: this.zsgcValue3,
					sourceTo: this.zsgcValue4,
					page: this.page,
					rows: this.rows
				};
				setTimeout(() => {
					_this.$http({
							method: 'get',
							url: `${_this.baseurl}NontightbookInfo/listPageJson`,
							params: temData
						})
						.then(function(res) {
			...
			update() {
				...
				let layerIndex = layer.open({
					...
					content: ['#/MaterialAccountWindow/' + id],
					end: function() {
						_this.getpage();
					}
				});
			...
			derive() {
				...
				_this.$http({
					method: 'get',
					url: '/bjsy-jdc/fs/NontightbookInfo/export',
					params: temData,
					responseType: 'blob'
				}).then(res => {
					_this.deriveAll(res);
				...
				layer.confirm(
					...
					function(index) {
						...
						_this
							.$http({
								method: "post",
								url: _this.baseurl + "NontightbookInfo/delete/" + id
							})
							.then(function(res) {
								_this.getpage();
							})
